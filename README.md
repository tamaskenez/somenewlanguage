...
There are no function or type declarations. Every user function and type exists as an anonymous function or type, which can optionally be bound to a variable.
...
By default all types are structurally typed. Nominal typing can be achieved by assigning a name to an anonymous type. This is different from assigning the type to a variable which does not change anything.
...
- For Sum types, there's automatic casting to supertype (superset of cases). It's automatic only for anonymous types (named types can be casted explicitly to anonymous then automatically to supertype).
...
All code can be forced to be executed in comptime (some restrictions might be needed regarding library usage). Some code might be automatically executed in comptime (simple arithmetics).
Function parameters are related to the comptime/runtime distinction in the following way:
- As a default, function parameters need a runtime argument whose type is known at comptime.
- Function parameters can be defined 'comptime' meaning that they need an argument whose value is also known at comptime.
- Function parameters can be defined 'optionally-comptime' which means they accept runtime arguments but if the argument is known in comptime it will create specializations of the function.

Any expression that needs to be comptime (all types and arguments flowing into comptime parameters) must be explicitly declared `comptime` unless it's already a comptime literal value.

    f1 = fn comptime x :: Int { ... } // Comptime parameter
    f1 3 // `comptime` declaration not needed
    f1 (comptime (3 + 3)) // `comptime` declaration needed

    f2 = fn x :: (comptime (b ? Int : String)) { ... } // Parameter's expected type is a comptime expression.
