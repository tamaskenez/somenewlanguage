...
#define MC store.MakeCanonical
    ...
    auto main_body_with_stdio = MC(term::Abstraction(
        vector<BoundVariable>({BoundVariable{stdio_variable, stdio_initializer},
                               BoundVariable{store.MakeNewVariable(make_copy(store.s_ignored_name)),
    ...
    auto main_lambda = MC(term::Abstraction(
        ...
        main_body));
    auto cimport_function = MC(term::Abstraction(
        vector<BoundVariable>(),
        vector<Parameter>({Parameter{store.MakeNewVariable("c_code"), store.string_literal_type}}),
        main_body));
